...
class Trader:
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in state.order_depths.keys():
            ...
            if product == 'BANANAS':
                ...
                if state.timestamp < 600:
                    break
                if asks[0]['price'] < acceptable_price or (
                        asks[0]['price'] == acceptable_price and banana_position < -2):
                ...
                elif bids[0]['price'] > acceptable_price or (
                        bids[0]['price'] == acceptable_price and banana_position > -5):
            ...
            if product == 'DIVING_GEAR':
                ...
                if len(order_depth_diving.sell_orders) > 0 and len(order_depth_diving.buy_orders) > 0:
                    mid_diving.append(
                        (max(order_depth_diving.buy_orders.keys()) + min(order_depth_diving.sell_orders.keys())) / 2)
        ...
        best_ask_volume_coconut = asks[0]['vol'];
        best_ask_coconut = asks[0]['price'];
        sellable_volume_coconut = 600 + coco_position;
        best_bid_volume_pina = bids_pina[0]['vol'];
        best_bid_pina = bids_pina[0]['price'];
        buyable_volume_pina = 300 - pina_position;
        best_bid_volume_coconut = bids[0]['vol'];
        best_ask_volume_pina = asks_pina[0]['vol'];
        buyable_volume_coconut = 600 - coco_position;
        sellable_volume_pina = 300 + pina_position;
        best_bid_coconut = bids[0]['price'];
        best_ask_pina = asks_pina[0]['price']
        window = 50
        ...
        print('cocopos', coco_position, 'pinapos', pina_position, 'sellablecoco', sellable_volume_coconut,
              'buyablecoco', buyable_volume_coconut)
